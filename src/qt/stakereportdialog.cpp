...
// format Silk value with all trailing zero
QString Coin_0Pad(int nUnit, CAmount amount)
{
    QString result = SilkUnits::format(nUnit, amount);

    int poin = result.indexOf(".") + 1;
    poin += SilkUnits::decimals(nUnit);
...
void StakeReportDialog::updateStakeReport(bool fImmediate=false)
{
    ...
    int nDisplayUnit = SilkUnits::SLK;
    if (ex_model && ex_model->getOptionsModel())
         nDisplayUnit = ex_model->getOptionsModel()->getDisplayUnit();

    ui->L_Coin->setText(SilkUnits::name(nDisplayUnit) + " " + tr("SubTotal"));

    QTableWidget *TableW = ui->StakeReportTable;

    TableW->horizontalHeaderItem(1)->setText(SilkUnits::name(nDisplayUnit) + " " +tr("Amount"));
    ...
    ui->Amount_24H->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" SLK"));
    ui->Stake_24H->setText(QString::number(aRange[i++].Count));
    ui->Amount_7D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" SLK"));
    ui->Stake_7D->setText(QString::number(aRange[i++].Count));
    ui->Amount_30D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" SLK"));
    ui->Stake_30D->setText(QString::number(aRange[i++].Count));
    ui->Amount_365D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" SLK"));
    ui->Stake_365D->setText(QString::number(aRange[i++].Count));

    ui->Amount_Last->setText(tr("Amount: ") + Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" SLK"));
